...
# ✅ **CORRECTION – EXERCICE 5 : Moyenne générale de la classe**

```c
float moyenneClasse(Etudiant T[], int n) {
    float s = 0;
    for (int i = 0; i < n; i++) {
        s += T[i].moy;
    }
...
# ✅ **CORRECTION – EXERCICE 7 : Tri par moyenne (tri simple par échange)**

```c
void trier(Etudiant T[], int n) {
    Etudiant temp;
...
# ✅ **CORRECTION – EXERCICE 9 : Moyenne pondérée**
...
float moyennePonderee(Matiere T[], int n) {
    float somCoef = 0, somNote = 0;
    for (int i = 0; i < n; i++) {
        somCoef += T[i].coef;
        somNote += T[i].note * T[i].coef;
    }
    return somNote / somCoef;
}
